...
To install Consensus:

    .. code-block:: python

        python -m pip install git+https://github.com/lb-lewisham/Consensus.git
...
gss.run_graph(starting_column='WD22CD', ending_column='LSOA21CD', geographic_areas=['Lewisham'], geographic_area_columns=['LAD21NM'])  # try changing WD22CD to WD21CD and WD23CD to see how this affects the results
codes = await gss.geodata(selected_path=0)  # the selected path is the ninth in the list of potential paths output by run_graph() method
print(codes['table_date'][0])
